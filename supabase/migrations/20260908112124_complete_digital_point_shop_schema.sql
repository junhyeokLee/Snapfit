-- Forward completion for a previously recorded empty migration. Keep existing
-- migration history and data; normal fresh installations already have this schema.
do $completion$
declare
  v_objects oid[] := array[
    to_regclass('public.point_shop_products')::oid,
    to_regclass('public.point_shop_ownership')::oid,
    to_regprocedure('public.get_point_shop_access(text)')::oid,
    to_regprocedure('public.purchase_point_shop_product(text,integer)')::oid,
    to_regprocedure('public.guard_point_shop_ownership()')::oid,
    to_regprocedure('public.guard_digital_point_ledger()')::oid
  ];
  v_present integer;
begin
  select count(object_id)::integer into v_present from unnest(v_objects) object_id;
  if v_present = 0 then
    execute $shop_schema$
...
$shop_schema$;
  elsif v_present <> cardinality(v_objects) then
    raise exception 'partial_point_shop_schema_requires_review' using errcode = '23514';
  end if;
end;
$completion$;
