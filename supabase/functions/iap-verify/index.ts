...
function productPlanCode(productId: string, requestedPlanCode: string) {
  ...
  if (configured) {
    try {
      return { ...defaultMap, ...JSON.parse(configured) }[productId] ??
        requestedPlanCode;
    } catch (_e) {
...
async function googleAccessToken() {
  ...
  if (!response.ok) {
    throw new Error(`google_oauth_failed:${JSON.stringify(data)}`);
  }
...
async function verifyGooglePlay(
  ...
  const packageName = text(
    body.packageName || Deno.env.get("GOOGLE_PLAY_PACKAGE_NAME"),
  );
  ...
  if (!response.ok) {
    throw new Error(
      `google_play_verify_failed:${response.status}:${JSON.stringify(data)}`,
    );
  }

  const lineItems = Array.isArray(data.lineItems) ? data.lineItems : [];
  const matchingItem = lineItems.find((item: Record<string, unknown>) => {
    const offerDetails = item?.offerDetails as
      | Record<string, unknown>
      | undefined;
    return text(
          item?.productId || offerDetails?.basePlanId || offerDetails?.offerId,
  ...
  return {
    ...
    productId,
    transactionId: text(body.transactionId) || text(data.latestOrderId) ||
      purchaseToken,
    originalTransactionId: text(body.originalTransactionId) ||
      text(data.latestOrderId) || purchaseToken,
    status,
    purchasedAt: start,
...
async function appleServerJwt() {
  ...
  if (!issuerId || !keyId || !bundleId || !privateKey) {
    throw new Error("app_store_credentials_not_configured");
  }
  const now = Math.floor(Date.now() / 1000);
...
async function verifyAppStore(
  ...
  if (expectedBundleId && bundleId !== expectedBundleId) {
    throw new Error("app_store_bundle_id_mismatch");
  }
  if (requestedProductId && productId !== requestedProductId) {
    throw new Error("app_store_product_id_mismatch");
  }
  ...
  const status: VerificationResult["status"] = revoked
    ? "REFUNDED"
    : expired
    ? "EXPIRED"
    : "VERIFIED";
...
async function mockVerification(
  body: Record<string, unknown>,
): Promise<VerificationResult> {
  const now = new Date();
  const expiresAt = new Date(now.getTime() + 30 * 24 * 60 * 60_000);
  const platform = text(body.platform).toUpperCase() as
    | "GOOGLE_PLAY"
    | "APP_STORE";
  return {
    platform,
...
async function verifyPurchase(body: Record<string, unknown>) {
  ...
  if (!text(body.productId) || !text(body.transactionId)) {
    throw new Error("productId_transactionId_required");
  }

  if (Deno.env.get("SNAPFIT_IAP_MOCK_VERIFY") === "true") {
    return mockVerification(body);
  }
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: pointProduct, error: pointProductError } = await supabase
      .from("point_products")
      .select("product_id,points")
      .eq("product_id", verification.productId)
      .eq("is_active", true)
      .maybeSingle();
    if (pointProductError) throw pointProductError;

    const isPointPurchase =
      text(body.purchaseType).toUpperCase() === "POINTS" ||
      Boolean(pointProduct);
    const requestedPlanCode = isPointPurchase
      ? "FREE"
      : text(body.planCode) || "SNAPFIT_PRO_MONTHLY";
    const planCode = isPointPurchase
      ? "FREE"
      : productPlanCode(verification.productId, requestedPlanCode);

    const { error: purchaseError } = await supabase.from("store_purchases")
      .upsert({
        user_id: user.id,
        ...
        fail_reason: verification.status === "VERIFIED"
          ? null
    ...
    if (isPointPurchase) {
      const { data: pointGrant, error: pointGrantError } = await supabase.rpc(
        "grant_point_purchase",
        {
          p_user_id: user.id,
          p_product_id: verification.productId,
          p_platform: verification.platform,
    ...
    const status = message.includes("Unauthorized")
      ? 401
      : message.includes("required") || message.includes("invalid_") ||
