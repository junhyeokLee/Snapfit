...
class _AddCoverScreenState extends ConsumerState<AddCoverScreen> {
  ...
  void initState() {
    ...
    _selectedCover = widget.initialCoverSize ?? coverSizes.firstWhere(
      (s) => s.name == '정사각형',
      ...
      if (!_initialized) {
        if (widget.editAlbum != null) {
          ...
          try {
            ...
            final album = await albumRepository.fetchAlbum(widget.albumId.toString());
          ...
          // + 버튼(신규 생성) 또는 플로우 진입: 빈 레이아웃으로 시작 (기존 데이터/갤러리 불러오기 X)
          ref
              .read(albumEditorViewModelProvider.notifier)
              .resetForCreate(initialCover: _selectedCover);
        }
