...
class SupabaseAlbumRepository implements AlbumRepository {
  ...
  Future<Album> createAlbum(CreateAlbumRequest request) async {
    ...
    final row = await client.from('albums').insert({
      'owner_id': userId,
      'title': request.title,
      'ratio': request.ratio,
  ...
  Future<Album> updateAlbum(int albumId, CreateAlbumRequest request) async {
    final userId = await _userId();
    final row = await client.from('albums').update({
      'title': request.title,
      'ratio': request.ratio,
      'cover_layers_json': request.coverLayersJson,
      'cover_image_url': request.coverImageUrl,
      ...
      'cover_theme': request.coverTheme,
      'target_pages': request.targetPages,
    }).eq('id', albumId).eq('owner_id', userId).select().single();
    return _fromRow(Map<String, dynamic>.from(row));
  }
  ...
  Future<List<Album>> fetchMyAlbums() async {
    ...
    return rows.map<Album>((row) => _fromRow(Map<String, dynamic>.from(row))).toList(growable: false);
  }

  @override
  Future<void> deleteAlbum(int albumId) async {
    final userId = await _userId();
    await client.from('albums').delete().eq('id', albumId).eq('owner_id', userId);
  }
  ...
  Future<void> lockAlbum(int albumId) async {
    final userId = await _userId();
    await client.from('albums').update({'locked_by_id': userId}).eq('id', albumId);
  }
